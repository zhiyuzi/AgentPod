...
├── naming.md          # Pod 命名由来
└── token-calc-report.md  # 火山引擎/智谱/MiniMax Token 计算能力调研
...
| WS | `/v1/edge/connect` | Edge Agent 反向 WebSocket 连接（Edge Gateway） |
...
- Edge Gateway（`edge.py` + `gateway/edge.py` + `tools/edge.py`）：Edge Agent 从用户本地通过 WebSocket 反向连接到云端 Gateway，EdgeConnectionManager 维护 `user_id → connection` 映射。AgenticLoop 每轮开始时调用 `_refresh_edge_tools()` 动态发现 Edge Agent 上的工具，注册为 `edge_` 前缀的 EdgeTool。Edge 配置位于共享层 `data/shared/.agents/config.toml`（TOML，全局控制，不支持用户级覆盖），`enabled = false` 可全局禁用特定工具
